...
public class Field extends JPanel implements MouseListener{
	...
	public Field(){
		...
		for(int i = 0; i < Data.FIELD_Y; i++){
			for(int j = 0; j < Data.FIELD_X; j++){
				...
				add(boxes[i][j]);
				if(i==Data.FIELD_Y-1 && j==0){
					boxes[i][j].setCard(new Player(Card.SOUTH));
				}
			}
		}
	}
	...
	public void clickField(MouseEvent e){
		...
		if(moveCost!=0) popup.add(move);
		if(attackCost!=0) popup.add(attack);
		if(activeSkillCost!=0) popup.add(activeSkill);
		if(evolveCost!=0 && 
		   !selectingCard.isEvolved() && 
		   ((selectingCard.getUser()==Card.SOUTH && selectingBox.getElement() == Box.ELEMENT.NORTH_FIELD) ||
		   (selectingCard.getUser()==Card.NORTH && selectingBox.getElement() == Box.ELEMENT.SOUTH_FIELD))
		) popup.add(evolution);
		popup.show(e.getComponent(), e.getX(),e.getY());
	}
